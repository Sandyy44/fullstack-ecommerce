...
export const profile = asyncHandler(async (req, res, next) => {
  ...
  if (finalUser.phone) finalUser.phone = decrypt({ encryptedText: userWishList.phone });
...
export const getUserById = asyncHandler(async (req, res, next) => {
  ...
  const user = await findOne({
    model: userModel,
    condition: { _id: id, status: { $ne: "blocked" } }, select: "-password -createdAt -updatedAt -changeTime -code -accountType -wishList"
  });
  if (!user) {
    return next(new Error("In-valid user", { cause: 404 }));
  }
...
export const users = asyncHandler(async (req, res, next) => {
  const users = await find({
    model: userModel,
    select: "-password -createdAt -updatedAt -changeTime -code  -wishList"
  });
  if (!users.length) {
